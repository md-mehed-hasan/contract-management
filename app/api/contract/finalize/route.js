...
export async function POST(request) {
  try {
    ...
    const contract = await Contract.findOne({ token: body.token }).populate('userId', 'email');
    ...
    const adminEmail = contract.userId?.email || process.env.ADMIN_EMAIL || 'admin@soas.com';
    ...
    await Promise.allSettled([
      ...
      sendAdminNotification(adminEmail, contract.clientName, contract.documentName, contract.signedDate.toISOString(), contract.signedPdfUrl)
    ]);
